...
На выбор: UEFI или Legacy

ВАЖНАЯ ИНФОРМАЦИЯ!

Вся разметка диска производится только в cfdisk!

В cfdisk необходимо указать для:

- UEFI:
    type=EFI для boot раздела
    type=linux - для корневого раздела\
    type=swap - для раздела подкачки, который должен быть равен размеру оперативной памяти
    type=linux - для home раздела
